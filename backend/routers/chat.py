...
async def legal_chat(message: str = Form(...), session_id: Optional[str] = Form(None), user_id: Optional[str] = Form(None)):
    try:
        ...
        for t, eng in LEGAL_SYNONYMS.items():
            if (" " in t.lower() and t.lower() in msg_text) or (" " not in t.lower() and t.lower() in cand_w):
                exp_kw.extend(eng)
        ...
        for i, t_text in enumerate(search_engine.title_corpus):
            ...
            if m_words:
                t_boost[i] = sum(search_engine.vocab_idf.get(w, 1.0) for w in m_words)

        # ── MULTI-EVIDENCE RECIPROCAL RANK FUSION (Joon Ho Lee, ACM SIGIR) ──
        def rrf(raw, k=60):
            r = np.empty_like(raw)
            r[np.argsort(-raw)] = np.arange(1, len(raw) + 1)
            return np.where(raw > 0, 1.0 / (k + r), 0.0)

        bge_rrf = rrf(bge_scores)
        bm25_rrf = rrf(bm25_scores)
        title_rrf = rrf(t_boost)
        f_scores = bge_rrf + bm25_rrf + title_rrf

        if float(np.max(bge_scores)) < 0.45 and dir_idx is None:
            return ChatResponse(response="Query not related to Labor Law.", session_id=session_id, laws=[])

        # ── MIN-MAX NORMALIZATION BOUNDS (Han et al., Montague & Aslam) ─────
        # Active evidence channels: BGE-M3 + BM25 (+ Title Boost if matching)
        k_val = 60.0
        max_per_channel = 1.0 / (k_val + 1.0)  # 1/61 ≈ 0.016393
        active_channels = 3.0 if np.max(t_boost) > 0 else 2.0
        
        # System's maximum possible output (Joon Ho Lee)
        x_max = active_channels * max_per_channel
        x_min = 0.0

        limit = (await db.settings.find_one({"key": "chat_limit"}) or {}).get("value", 5)
        matched = []

        # Exact Article Number Match -> Direct 100% confidence
        if dir_idx is not None:
            ...
            d_law['relevance_pct'] = 100  # Exact article query hit
            matched.append(d_law)

        # Ranked Hybrid Matches -> Min-Max Normalized: X_norm = (X - X_min) / (X_max - X_min)
        for i in np.argsort(f_scores)[::-1]:
            ...
            if f_scores[i] > 0.01 and bge_scores[i] >= 0.35 and (q_set & set(search_engine.corpus[i].split())):
                ...
                raw_score = float(f_scores[i])
                x_norm = (raw_score - x_min) / (x_max - x_min)
                norm_pct = min(max(round(x_norm * 100), 1), 100)

                ld['relevance_pct'] = norm_pct
                matched.append(ld)

        resp = f"I found {len(matched)} relevant article(s) regarding your query:" if matched else "No specific laws found."
...
# ── Session endpoints (deduplicated — keep only ONE of each) ──
...
async def get_sessions(user_id: str, skip: int = 0, limit: int = 20):
    pipeline = [
        ...
        {"$group": {
            ...
            "timestamp":    {"$max":   "$created_at"}   # use max so the group's date = latest message
        }},
        {"$sort": {"timestamp": -1}},    # newest sessions first — applied BEFORE paging
        {"$facet": {
            "total":    [{"$count": "count"}],
            "sessions": [{"$skip": skip}, {"$limit": limit}],  # correct page of sorted sessions
